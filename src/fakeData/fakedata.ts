import { VideoType } from "../pages/MainPage";

export const initialVideoSrc =
  "https://media.istockphoto.com/id/1392123838/es/v%C3%ADdeo/alimentaci%C3%B3n-de-desplazamiento-manual-en-el-tel%C3%A9fono-inteligente-con-pantalla-verde-mock-up.mp4?s=mp4-640x640-is&k=20&c=SZIcYR4LoKZjrzBfNEGA3uAbtvj5CLx-uRKnMcYERxw=";

export const finalVideoSrc =
  "https://media.istockphoto.com/id/1392123838/es/v%C3%ADdeo/alimentaci%C3%B3n-de-desplazamiento-manual-en-el-tel%C3%A9fono-inteligente-con-pantalla-verde-mock-up.mp4?s=mp4-640x640-is&k=20&c=SZIcYR4LoKZjrzBfNEGA3uAbtvj5CLx-uRKnMcYERxw=";

export const fakeVideos: VideoType[] = [
  {
    index: 0,
    src: "https://media.istockphoto.com/id/1392123838/es/v%C3%ADdeo/alimentaci%C3%B3n-de-desplazamiento-manual-en-el-tel%C3%A9fono-inteligente-con-pantalla-verde-mock-up.mp4?s=mp4-640x640-is&k=20&c=SZIcYR4LoKZjrzBfNEGA3uAbtvj5CLx-uRKnMcYERxw=",
    profilePic:
      "https://cdn.pixabay.com/photo/2017/02/23/13/05/avatar-2092113_1280.png",
    caption: "Video caption 1... de uno",
    username: "colacados",
    comments: [
      {
        username: "username",
        comment:
          "Las personas solo pensando en ayudas y ayudas, pero no en trabajar...",
        date: "6 d",
        likes: 24,
      },
      {
        username: "username",
        comment:
          "Las personas solo pensando en ayudas y ayudas, pero no en trabajar...",
        date: "6 d",
        likes: 24,
      },
      {
        username: "username",
        comment:
          "Las personas solo pensando en ayudas y ayudas, pero no en trabajar...",
        date: "6 d",
        likes: 24,
      },
      {
        username: "username",
        comment:
          "Las personas solo pensando en ayudas y ayudas, pero no en trabajar...",
        date: "6 d",
        likes: 24,
      },
      {
        username: "username",
        comment:
          "Las personas solo pensando en ayudas y ayudas, pero no en trabajar...",
        date: "6 d",
        likes: 24,
      },
      {
        username: "username",
        comment:
          "Las personas solo pensando en ayudas y ayudas, pero no en trabajar...",
        date: "6 d",
        likes: 24,
      },
      {
        username: "edgarflores425",
        comment: "Que chido video",
        date: "5 d",
        likes: 12,
      },
      {
        username: "edgarflores425",
        comment: "Que chido video",
        date: "5 d",
        likes: 12,
      },
      {
        username: "edgarflores425",
        comment: "Que chido video",
        date: "5 d",
        likes: 12,
      },
      {
        username: "edgarflores425",
        comment: "Que chido video",
        date: "5 d",
        likes: 12,
      },
      {
        username: "edgarflores425",
        comment: "Que chido video",
        date: "5 d",
        likes: 12,
      },
      {
        username: "edgarflores425",
        comment: "Que chido video",
        date: "5 d",
        likes: 12,
      },
    ],
  },
  {
    index: 1,
    src: "https://media.istockphoto.com/id/1392123838/es/v%C3%ADdeo/alimentaci%C3%B3n-de-desplazamiento-manual-en-el-tel%C3%A9fono-inteligente-con-pantalla-verde-mock-up.mp4?s=mp4-640x640-is&k=20&c=SZIcYR4LoKZjrzBfNEGA3uAbtvj5CLx-uRKnMcYERxw=",
    profilePic:
      "https://cdn.pixabay.com/photo/2016/08/08/09/17/avatar-1577909_1280.png",
    caption:
      "Otra descripción... bla bla bla, me gustan las patatas, bla bla bla. Esto es un texto muy largo para ver como se comporta el texto en el video.",
    username: "username",
    comments: [
      {
        username: "username",
        comment:
          "Las personas solo pensando en ayudas y ayudas, pero no en trabajar...",
        date: "6 d",
        likes: 24,
      },
      {
        username: "edgarflores425",
        comment: "Que chido video",
        date: "5 d",
        likes: 12,
      },
    ],
  },
  {
    index: 2,
    src: "https://media.istockphoto.com/id/1392123838/es/v%C3%ADdeo/alimentaci%C3%B3n-de-desplazamiento-manual-en-el-tel%C3%A9fono-inteligente-con-pantalla-verde-mock-up.mp4?s=mp4-640x640-is&k=20&c=SZIcYR4LoKZjrzBfNEGA3uAbtvj5CLx-uRKnMcYERxw=",
    profilePic:
      "https://static.vecteezy.com/system/resources/previews/021/548/095/non_2x/default-profile-picture-avatar-user-avatar-icon-person-icon-head-icon-profile-picture-icons-default-anonymous-user-male-and-female-businessman-photo-placeholder-social-network-avatar-portrait-free-vector.jpg",
    caption: "porque así es la vida como la pintan... asdf asdf a asdf asd",
    username: "perico",
    comments: [
      {
        username: "username",
        comment:
          "Las personas solo pensando en ayudas y ayudas, pero no en trabajar...",
        date: "6 d",
        likes: 24,
      },
      {
        username: "edgarflores425",
        comment: "Que chido video",
        date: "5 d",
        likes: 12,
      },
    ],
  },
  {
    index: 3,
    src: "https://media.istockphoto.com/id/1392123838/es/v%C3%ADdeo/alimentaci%C3%B3n-de-desplazamiento-manual-en-el-tel%C3%A9fono-inteligente-con-pantalla-verde-mock-up.mp4?s=mp4-640x640-is&k=20&c=SZIcYR4LoKZjrzBfNEGA3uAbtvj5CLx-uRKnMcYERxw=",
    profilePic:
      "https://t3.ftcdn.net/jpg/05/16/27/58/360_F_516275801_f3Fsp17x6HQK0xQgDQEELoTuERO4SsWV.jpg",
    caption: "porque así es la vida como la pintan... asdf asdf a asdf asd",
    username: "de los palotes",
    comments: [
      {
        username: "username",
        comment:
          "Las personas solo pensando en ayudas y ayudas, pero no en trabajar...",
        date: "6 d",
        likes: 24,
      },
      {
        username: "edgarflores425",
        comment: "Que chido video",
        date: "5 d",
        likes: 12,
      },
    ],
  },
  {
    index: 4,
    src: "https://media.istockphoto.com/id/1392123838/es/v%C3%ADdeo/alimentaci%C3%B3n-de-desplazamiento-manual-en-el-tel%C3%A9fono-inteligente-con-pantalla-verde-mock-up.mp4?s=mp4-640x640-is&k=20&c=SZIcYR4LoKZjrzBfNEGA3uAbtvj5CLx-uRKnMcYERxw=",
    profilePic:
      "https://st2.depositphotos.com/2559749/11304/v/450/depositphotos_113040644-stock-illustration-flat-icon-isolate-on-white.jpg",
    caption: "porque así es la vida como la pintan... asdf asdf a asdf asd",
    username: "miau miau",
    comments: [
      {
        username: "username",
        comment:
          "Las personas solo pensando en ayudas y ayudas, pero no en trabajar...",
        date: "6 d",
        likes: 24,
      },
      {
        username: "edgarflores425",
        comment: "Que chido video",
        date: "5 d",
        likes: 12,
      },
    ],
  },
  {
    index: 5,
    src: "https://media.istockphoto.com/id/1392123838/es/v%C3%ADdeo/alimentaci%C3%B3n-de-desplazamiento-manual-en-el-tel%C3%A9fono-inteligente-con-pantalla-verde-mock-up.mp4?s=mp4-640x640-is&k=20&c=SZIcYR4LoKZjrzBfNEGA3uAbtvj5CLx-uRKnMcYERxw=",
    profilePic:
      "https://cdn.pixabay.com/photo/2017/02/23/13/05/avatar-2092113_1280.png",
    caption: "porque así es la vida como la pintan... asdf asdf a asdf asd",
    username: "1980 54",
    comments: [
      {
        username: "username",
        comment:
          "Las personas solo pensando en ayudas y ayudas, pero no en trabajar...",
        date: "6 d",
        likes: 24,
      },
      {
        username: "edgarflores425",
        comment: "Que chido video",
        date: "5 d",
        likes: 12,
      },
    ],
  },
  {
    index: 6,
    src: "https://media.istockphoto.com/id/1392123838/es/v%C3%ADdeo/alimentaci%C3%B3n-de-desplazamiento-manual-en-el-tel%C3%A9fono-inteligente-con-pantalla-verde-mock-up.mp4?s=mp4-640x640-is&k=20&c=SZIcYR4LoKZjrzBfNEGA3uAbtvj5CLx-uRKnMcYERxw=",
    profilePic:
      "https://microbiology.ucr.edu/sites/default/files/styles/form_preview/public/blank-profile-pic.png?itok=4teBBoet",
    caption: "porque así es la vida como la pintan... asdf asdf a asdf asd",
    username: "xd xd xd",
    comments: [
      {
        username: "username",
        comment:
          "Las personas solo pensando en ayudas y ayudas, pero no en trabajar...",
        date: "6 d",
        likes: 24,
      },
      {
        username: "edgarflores425",
        comment: "Que chido video",
        date: "5 d",
        likes: 12,
      },
    ],
  },
];
